    ...
    def validate_values(cls, data: Any):
        if isinstance(data, dict):
            ...
            if not str(data.get("members_count")).isnumeric():
                raise Exception(
                    "chatroom cache members_count value is not a valid integer."
                )
            ...
            if not modified_at_is_valid:
                raise Exception(
                    "chatroom cache modified_at value is not a valid float."
                )
            ...
            if creator_uid:
                ...
                if not creator_uid_is_valid:
                    raise Exception(
                        "chatroom cache creator_uid value is not a valid uid."
                    )
            ...
            if creator_successor_uid:
                ...
                if not creator_successor_uid_is_valid:
                    raise Exception(
                        "chatroom cache creator_successor_uid value is not a valid uid."
                    )
