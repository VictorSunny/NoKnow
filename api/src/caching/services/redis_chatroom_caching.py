...
async def set_chatroom_modified_at_cache(id: UUID, r_client: redis.Redis):
    # fix hash key
    await r_client.hset(
        Config.REDIS_CHATROOM_MODIFICATION_DATE_LIST_NAME, str(id), str(timestamp_now())
    )
...
async def check_chatroom_secret_mode_active(id: UUID, r_client: redis.Redis):
    chatroom_is_secret = await r_client.sismember(
        Config.REDIS_SECRET_CHATROOMS_LIST_NAME, str(id)
    )
...
async def check_record_message(id: UUID, r_client: redis.Redis):
    chatroom_is_secret = await check_chatroom_secret_mode_active(
        id=id, r_client=r_client
    )
    if chatroom_is_secret:
        return False
    return True
...
async def set_chatroom_cache(chatroom: Chatroom, r_client: redis.Redis):
    ...
    await r_client.hset(
        f"{Config.REDIS_CHATROOM_NAME_PREFIX}:{str(chatroom.uid)}", mapping=data
    )
    logger.info(f"succesfully set cache for chatroom with id: {chatroom.uid}")


async def get_chatroom_from_cache(
    chatroom_identifier: UUID | str, r_client: redis.Redis
):
    """
