...
async def user_join_chatroom(
    ...
    join_chat_response = await join_chatroom(
        user=user, id=id, chatroom_password=password, db=db, r_client=r_client
    )
...
async def remove_and_ban_user(
    ...
    response = await remove_and_ban_user_from_chat(
        id=id, violator_uid=user_uid, user=user, db=db, r_client=r_client
    )
    return response
...
async def add_and_unban_user(
    ...
    response = await add_and_unban_user_from_chat(
        id=id, forgiven_uid=user_uid, user=user, db=db, r_client=r_client
    )
    return response
...
async def add_user_to_moderators(
    ...
    response = await add_user_to_chatroom_moderators(
        id=id, candidate_uid=user_uid, user=user, db=db, r_client=r_client
    )
    return response
...
async def make_user_chatroom_successor(
    ...
    response = await assign_chatroom_successor(
        candidate_uid=user_uid, id=id, user=user, db=db, r_client=r_client
    )
    return response
